defmodule LazyFor.KeywordOptions.Test do
  ...
  test "for comprehensions with unique values" do
    ...
    assert Enum.to_list(stream(x <- list, uniq: true, into: "", do: to_bin(x * 2))) == <<2, 4, 6>>
    assert Enum.to_list(stream(<<x <- "abcabc">>, uniq: true, into: "", do: to_bin(x))) == "abc"
    ...
    assert_raise RuntimeError, "oops", fn ->
      stream(_ <- [1, 2, 3], uniq: true, into: %Pdict{}, do: raise("oops"))
    end
  ...
  test "for comprehensions with binary, enum generators and filters" do
    assert Enum.to_list(stream(x <- [1, 2, 3], <<(y <- <<4, 5, 6>>)>>, y / 2 == x, do: x * y)) ==
             [8, 18]
  end
  ...
  test "binary for comprehensions" do
    bin = <<1, 2, 3>>
    assert Enum.to_list(stream(<<x <- bin>>, do: x * 2)) == [2, 4, 6]
  end
  ...
  test "binary for comprehensions with two generators" do
    assert Enum.to_list(
             stream(<<(x <- <<1, 2, 3>>)>>, <<(y <- <<4, 5, 6>>)>>, y / 2 == x, do: x * y)
           ) == [8, 18]
  end

  test "binary for comprehensions into list" do
    bin = <<1, 2, 3>>
    assert Enum.to_list(stream(<<x <- bin>>, into: [], do: x * 2)) == [2, 4, 6]
  end
